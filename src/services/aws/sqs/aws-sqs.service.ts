...
export namespace AWSSQSService {
   ...
   */
  export const sendMessage = async <T>(queueName: EQueueName, body: T, delaySeconds?: number): Promise<boolean> => {
    ...
    try {
      ...
      await sqsClient
        ...
        .then((data) => {
          sendResult = true
          LogService.write(ELogLevel.DEBUG, 'AWSSQSService::sendMessage::SUCCESS', data)
        })
        .catch((error) => {
          sendResult = false
          LogService.write(ELogLevel.ERROR, 'AWSSQSService::sendMessage::ERROR', error)
        })
